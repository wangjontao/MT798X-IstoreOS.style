...
sed -i 's/192.168.[0-9]*.[0-9]*/192.168.9.1/g' package/base-files/files/bin/config_generate

# 自定义主机名
#sed -i "s/hostname='ImmortalWrt'/hostname='DulWiFi Live'/g" package/base-files/files/bin/config_generate

# 固件版本名称自定义
#sed -i "s/DISTRIB_DESCRIPTION=.*/DISTRIB_DESCRIPTION='DulWiFi TikTok Live By gino $(date +"%Y%m%d")'/g" package/base-files/files/etc/openwrt_release
...
# === 修复 OpenWrt 24.10 Rust 编译失败（Actions 环境 stage0 cargo + Cargo.toml.orig 丢失） ===
